...
function routes(app) {
	...
	app.all('/keystone/settings', function(req, res, next) {
		...
		req.params.item = keystone.get('settingsId');
		next();
	...
	app.all('/keystone/:list/:page([0-9]{1,5})?', userHandling.authPath, initList(true), userHandling.authObj, require('../../admin/routes/views/list'));
	app.all('/keystone/:list/:item', userHandling.authPath, initList(true), userHandling.authObj, require('../../admin/routes/views/item'));
