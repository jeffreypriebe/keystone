...
var userHandling = require('./userHandler.js');
...
function routes(app) {
	...
	// Admin-UI TinyMCE base for plugins
	app.all('/keystone/tiny-mce-plugin/:plugin', require('../../admin/routes/views/tiny-mce-plugin'));

	// Admin-UI API

	// A single Settings that isn't allowed to be created
	const settingsId = keystone.get('settingsId');
	if (settingsId) {
		app.all('/keystone/settings', function(req, res, next) {
			req.params.list = 'settings';
			req.params.item = settingsId;
			next();
		}, initList(true), require('../../admin/routes/views/item'));
		app.all('/keystone/settings/:item', function(req, res, next) {
			res.redirect('/keystone/settings');
		});
	}
	//Default list routes and block on user admins from editing users
	app.all('/keystone/:list/:page([0-9]{1,5})?', initList(true), userHandling.authObj, require('../../admin/routes/views/list'));
	app.all('/keystone/:list/:item', initList(true), userHandling.authObj, require('../../admin/routes/views/item'));
